...
### Profile switching (in-app, single Firebase identity)

A primary supervisor can switch their app's vantage point to view as any `managed_client` or `device_client` in the same circle without changing their Firebase auth identity. The state is local-only (the `acting_person_id` UserDefaults key, owned by `ProfileSwitchCoordinator` and surfaced as `AuthService.actingPersonID` / `actorPerson`), persists across relaunch, and is forced clear if the target is deleted, the supervisor is themselves demoted mid-switch, or the session ends. Dose logs written during act-as attribute `personID` to the target (whose dose it is — derived from the medication) and `loggedByPersonID` to the supervisor (who actually tapped) — this preserves audit accuracy. Rules-side: no change — the Firebase UID stays the supervisor's; permissions are unchanged. The switch is purely a UI lens: routing, role gates, and whose-data reads go through `actorPerson`; write attribution and "can I start a switch" checks stay on `currentPerson`.
